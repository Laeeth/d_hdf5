...
void soft_link_example()
{
     ...
     * callback.
     */
    const H5L_class_t[1] UD_soft_class = [{
        ...
        cast(H5L_delete_func_t)0,                       /* Move callback                  */
        cast(H5L_move_func_t)0,                       /* Copy callback                  */
        &UD_soft_traverse,           /* The actual traversal function  */
        cast(H5L_delete_func_t)0,             /* Deletion callback              */
...
extern(C) void plist_link_example()
{
    hid_t file_id;
    hid_t group_id, group2_id;
    hid_t gapl_id;
     ...
     * callback.
     */
    const H5L_class_t[1] UD_plist_class = [{
        H5L_LINK_CLASS_T_VERS,      /* Version number for this struct.
                                     * This field is always H5L_LINK_CLASS_T_VERS */
        ...
        cast(H5L_create_func_t)0;  /* Callback during link creation        */
        cast(H5L_move_func_t)0;      /* Callback after moving link           */
        cast(H5L_copy_func_t)0;      /* Callback after copying link          */
        cast(H5L_traverse_func_t)0;  /* Callback during link traversal       */
        cast(H5L_delete_func_t)0;     /* Callback for link deletion           */
        cast(H5L_query_func_t)0;    /* Callback for queries                 */
    }];
    ...
    H5L.register(&UD_plist_class[0]);
    H5L.create_ud(file_id, "plist_link", cast(H5LType)UD_PLIST_CLASS, cast(void*)0, 0, H5P_DEFAULT, H5P_DEFAULT);
     ...
     */
    H5P.insert2(gapl_id, PLIST_LINK_PROP, (const char *).sizeof, path,         cast(H5L_create_func_t)0,  /* Callback during link creation        */
        cast(H5L_move_func_t)0,      /* Callback after moving link           */
        cast(H5L_copy_func_t)0,      /* Callback after copying link          */
        cast(H5L_traverse_func_t)0,  /* Callback during link traversal       */
        cast(H5L_delete_func_t)0,     /* Callback for link deletion           */
 ...
 */
 hid_t UD_plist_traverse(const char *link_name, hid_t cur_group, const void *udata, size_t udata_size, hid_t lapl_id)
{
    char *        path;
    hid_t         ret_value = -1;
