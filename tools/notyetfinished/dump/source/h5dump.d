...
enum  H5DUMP_MAX_RANK    =H5S_MAX_RANK;

enum  ATTRIBUTE_DATA =0;
enum  DATASET_DATA    =1;
enum  ENUM_DATA       =2;
enum  COL             =3;
...
enum  PACKED_BITS_MAX         8  /* Maximum number of packed-bits to display */
enum  PACKED_BITS_SIZE_MAX    8*sizeof(long long)  /* Maximum bits size of integer types of packed-bits */
/* mask list for packed bits */
ulong  long packed_mask[PACKED_BITS_MAX];  /* packed bits are restricted to 8*sizeof(llong) bytes */
...
enum  DEFAULT_DTD     "http://www.hdfgroup.org/HDF5/XML/DTD/HDF5-File.dtd"
...
parse_mask_list(const char *h_list)
{
    ...
    while (*ptr) {
        ...
        temp_mask = ~0L;
        if(length_value<8*sizeof(ulong  long)) {
